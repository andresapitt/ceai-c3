...
| ~~**No rate limiting**~~ **CLOSED, with a caveat** | 10 requests a minute and 50 an hour per visitor, plus 150 a minute across the instance. Tested with a fake clock: a realistic session of 8 questions is never blocked, a burst of 20 gets exactly 10 through, the window rolls correctly, one visitor being limited does not affect another, and the hourly cap holds across twelve minutes. **The caveat:** Vercel runs several instances, each with its own memory, and a cold start resets the counters, so a flood spread across instances gets more than these numbers. It reliably stops the realistic case, which is one script or one tab hammering the endpoint |
| **An exact limit would need a shared store** | Vercel KV or Upstash would make the cap precise across instances. It also adds a paid service and another credential to an organisation with no engineer. Worth doing when the traffic justifies it, not before |
